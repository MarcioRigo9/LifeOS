...
import { proposeMealPlanActivation } from "@/lib/nutrition/mealPlans";

const bodySchema = z.object({ householdId: z.string().uuid(), mealPlanId: z.string().uuid() });
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const proposal = await proposeMealPlanActivation(getRuntimePool(), { householdId, userId, mealPlanId: parsed.data.mealPlanId });
